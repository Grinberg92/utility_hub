...
GLOBAL_CONFIG = {
    ...
    "patterns": {
        ...
        "compare_versions_shot_versions_mask": r'(?<!\d)(?:[a-zA-Z]+_)*\d{3,4}[a-zA-Z]?_\d{1,4}(?:_[a-zA-Z]+)*?_[vV]?\d+\w*(?!\d)',  # Имя с версией 001_0010_comp_v001 или prk_001_0010_comp_v001
        "compare_versions_shot_no_versions_mask": r'(?<!\d)(?:..._)?\d{3,4}[a-zA-Z]?_\d{1,4}(?!\d)', # Короткое имя только prk_001_0010, 001_0010, 001_001c
        "compare_versions_shot_soft_mask": r'(.+_)?\d{1,4}[a-zA-Z]?_\d{1,4}_.+', # Легкая маска, для отбрасывания .exr файлов которые не относятся к шотам. Например титры
