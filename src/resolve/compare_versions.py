...
class VersionCheckerApp:
    def __init__(self, root):
        ...
        tk.Label(root, text="Выберите файл данных:", font=("Arial", 13, "bold")).pack()
        tk.Entry(root, textvariable=self.file_path, width=50).pack()
        tk.Button(root, text="Обзор", command=self.select_file).pack()
        
        tk.Label(root, text="Выберите путь для сохранения результата:", font=("Arial", 13, "bold")).pack()
        tk.Entry(root, textvariable=self.result_path, width=50).pack()
        tk.Button(root, text="Обзор", command=self.select_result_path).pack()

        tk.Label(root, text="Источник данных:", font=("Arial", 13, "bold")).pack()
        ...
        tk.Label(root, text="Выбор рила:", font=("Arial", 13, "bold")).pack(pady=3)
        reel_number = tk.Frame(root)
        reel_number.pack()
        tk.Label(reel_number, text="Номер:").pack(side=tk.LEFT)
        tk.Entry(reel_number, textvariable=self.reel_name, width=5).pack(side=tk.LEFT)

        tk.Label(root, text="Диапазон треков Davinci Resolve", font=("Arial", 13, "bold")).pack(pady=3)
        track_frame = tk.Frame(root)
        track_frame.pack()
        tk.Label(track_frame, text="Начало:").pack(side=tk.LEFT)
        tk.Entry(track_frame, textvariable=self.start_track, width=5).pack(side=tk.LEFT, padx=5)
        tk.Label(track_frame, text="Конец:").pack(side=tk.LEFT)
        tk.Entry(track_frame, textvariable=self.end_track, width=5).pack(side=tk.LEFT)

        tk.Label(root, text="Ввод данных из Excel (Если выбран Excel)", font=("Arial", 13, "bold")).pack(pady=3)
        sheet_frame = tk.Frame(root)
        sheet_frame.pack()
        tk.Label(sheet_frame, text="Лист:").pack(side=tk.LEFT)
        self.sheet = tk.Entry(sheet_frame, textvariable=self.sheet_name, width=15)
        self.sheet.pack(side=tk.LEFT, padx=5)
        tk.Label(sheet_frame, text="Рил:").pack(side=tk.LEFT)
        self.reel = tk.Entry(sheet_frame, textvariable=self.column_reel, width=5)
        self.reel.pack(side=tk.LEFT, padx=5)
        tk.Label(sheet_frame, text="Шоты:").pack(side=tk.LEFT)
        self.shots = tk.Entry(sheet_frame, textvariable=self.column_letter, width=5)
        self.shots.pack(side=tk.LEFT, padx=5)
        
        self.result_label = tk.Label(root, text="Проверено 0 шотов из 0", font=("Arial", 17, "bold"))
        self.result_label.pack()
        tk.Button(root, text="Запустить", command=self.run_script,  width=30, height=1).pack(pady=5)
